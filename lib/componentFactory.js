  ...
  switch (element[0].name) {
    ...
    case this.components.center:
      if (element.children().length > 0) {
        var blockLevelElements = ['ADDRESS', 'BLOCKQUOTE', 'CENTER', 'DIR', 'DIV', 'DL', 'FIELDSET', 'FORM', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'HR', 'ISINDEX', 'MENU', 'NOFRAMES', 'NOSCRIPT', 'OL', 'P', 'PRE', 'TABLE', 'UL', 'DD', 'DT', 'FRAMESET', 'LI', 'TBODY', 'TD', 'TFOOT', 'TH', 'THEAD', 'TR'];
        element.children().each(function() {
          if (blockLevelElementsList.includes($(this).prop('tagName'))) {
            // Add `align="center"` attribute to block-level elements that are children of the `<center>` element
      ...
      // Add the class "data-parsed" to the `<center>` element (rather than a data attribute, which is not valid XHTML 1.0)
      element.addClass('data-parsed');
    ...
    case this.components.spacer:
      ...
      if (element.attr('size-sm') || element.attr('size-lg')) {
        if (element.attr('size-sm')) {
          ...
          html += '<table class="%s hide-for-large"><tbody><tr><td %s height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&#xA0;</td></tr></tbody></table>';
        }
        if (element.attr('size-lg')) {
          size = (element.attr('size-lg'));
          html += '<table class="%s show-for-large"><tbody><tr><td %s height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&#xA0;</td></tr></tbody></table>';
        }
      } else {
        size = (element.attr('size')) || 16;
        html += '<table class="%s"><tbody><tr><td %s height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&#xA0;</td></tr></tbody></table>';
      }

      // If size-sm and size-lg are present, `html` will contain two spacers (thus, provide the format strings twice)
      if( element.attr('size-sm') && element.attr('size-lg') ) {
        return format(html, classes.join(' '), attrs, classes.join(' '), attrs, inner);
      }

      return format(html, classes.join(' '), attrs, inner);
