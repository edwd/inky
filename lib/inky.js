 ...
 * @param {object|string} html Source XML/HTML, with Inky markup, as a cheerio object or a string
 * @param {object} cheerioOpts Options for cheerio, which are defined by htmlparser2's options (see: https://github.com/fb55/htmlparser2/wiki/Parser-options)
...
Inky.prototype.releaseTheKraken = function(html, cheerioOpts) {
  ...
  const $ = cheerio.load(html, Inky.mergeCheerioOpts(cheerioOpts));
  console.log($.html());
...
/**
 * Merges an options object with the defined defaults and returns the result. (In particular, if `decodeEntities` is not present, adds `decodeEntities: false`.)
 * @param {object} opts
 * @returns {object}
 */
