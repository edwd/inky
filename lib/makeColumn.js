...
module.exports = function(column) {
  ...
  const largeSize = parseInt($(column).attr('large'), 10) || parseInt($(column).attr('small'), 10) || Math.floor(this.columnCount / columnsInCurrentGroup);

  // The noExpander flag is `true` when the `no-expander` attribute is present on this `<column>` and does not have a value of `"false"`
  const noExpander = (typeof $(column).attr('no-expander') !== 'undefined' &&  $(column).attr('no-expander') !== 'false');
   ...
   */
  if (largeSize === this.columnCount && column.find('.row, row').length === 0 && noExpander === false ) {
